...
SCMD(kz_globalcheck, SCFL_GLOBAL | SCFL_MAP | SCFL_PLAYER)
{
	...
	if (KZGlobalService2::IsAvailable())
	{
		status.apiOnline = true;
		status.serverConnected = true;
		status.mapGlobal =
			KZGlobalService2::WithCurrentMap([](const std::optional<KZ::api::messages::MapInfo> &mapInfo) { return mapInfo.has_value(); });
		status.playerGlobal = !player->globalService2->IsBanned();
		KZGlobalService2::WithGlobalModes(
			[&](const KZGlobalService2::GlobalModeChecksums &checksums)
			{
